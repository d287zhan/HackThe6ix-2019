...
public class Pirate extends EnemyObject {
    ...
    public void render(Graphics2D g) {
        ...
        g.fillRect((int) x, (int) y - 20, (int) (200 * (health * 1.0 / 100)), 15);
    }
        ...
        if (!App.hud.showText) {
            if (!HUD.isDead) {
                if (shootingTimer > 0) {
                    ...
                    Game.handler.addSpecialBlocks(new Projectile((int) x, (int) y, ObjectIds.Block, BlockId.Orange,
                            rand.nextInt(6) + 4, rand.nextInt(6) + 4));
                    Game.handler.addSpecialBlocks(new Projectile((int) x, (int) y, ObjectIds.Block, BlockId.Orange,
                            rand.nextInt(6) + 4, rand.nextInt(6) + 4));
                    shootingTimer = 120;
                }
            ...
            for (int i = 0; i < Game.handler.sBlocks.size(); i++) {
                ...
                if (block.getBlockId().equals(BlockId.Orange) && ((Projectile) block).bounced
                        && block.getBounds().intersects(getBounds())) {
                    health--;
